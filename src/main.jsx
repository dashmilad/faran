...
function App() {
  ...
  const total = useMemo(
    () =>
      rows.reduce(
        (sum, row) =>
          sum + (Number(String(row.amount).replace(/,/g, '')) || 0),
        0
      ),
    [rows]
  );
  ...
  // با تغییر تاریخ اصلی فرم،
  // تاریخ تمام ردیف‌ها نیز خودکار تغییر می‌کند.
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      pdf.save(
        `فرم-هزینه-${header.date || 'بدون-تاریخ'}.pdf`
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="control-panel no-print">
        ...
        <p className="hint">
          اطلاعات را وارد کنید؛ فرم سمت چپ/پایین دقیقاً برای چاپ A4 آماده می‌شود.
        </p>
